<template>
  <view class="vw-cascade-selection">
    <scroll-view scroll-x scroll-with-animation :scroll-into-view="scrollViewId" :style="{ backgroundColor: headerBgColor }" class="vw-bottom-line" :class="{ 'vw-btm-none': !headerLine }">
      <view class="vw-selection-header" :style="{ height: tabsHeight, backgroundColor: backgroundColor }">
        <view class="vw-header-item" :class="{ 'vw-font-bold': index === currentTab && bold }" :style="{ color: index === currentTab ? activeColor : color, fontSize: size + 'rpx' }" :id="`id_${index}`" @tap.stop="swichNav" :data-current="index" v-for="(item, index) in selectedArr" :key="index">
          {{ item.text }}
          <view class="vw-active-line" :style="{ backgroundColor: lineColor }" v-if="index === currentTab && showLine"></view>
        </view>
      </view>
    </scroll-view>
    <swiper class="vw-selection-list" :current="currentTab" duration="300" @change="switchTab" :style="{ height: height, backgroundColor: backgroundColor }">
      <swiper-item v-for="(item, index) in selectedArr" :key="index">
        <scroll-view scroll-y :scroll-into-view="item.scrollViewId" class="vw-selection-item" :style="{ height: height }">
          <view class="vw-first-item" :style="{ height: firstItemTop }"></view>
          <view class="vw-selection-cell" :style="{ padding: padding, backgroundColor: backgroundColor }" :id="`id_${subIndex}`" v-for="(subItem, subIndex) in item.list" :key="subIndex" @tap="change(index, subIndex, subItem)">
            <icon type="success_no_circle" v-if="item.index === subIndex" :color="checkMarkColor" :size="checkMarkSize" class="vw-icon-success"></icon>
            <image :src="subItem.src" v-if="subItem.src" class="vw-cell-img" :style="{ width: imgWidth, height: imgHeight, borderRadius: radius }"></image>
            <view class="vw-cell-title" :class="{ 'vw-font-bold': item.index === subIndex && textBold, 'vw-flex-shrink': nowrap }" :style="{ color: item.index === subIndex ? textActiveColor : textColor, fontSize: textSize + 'rpx' }">
              {{ subItem.text }}
            </view>
            <view class="vw-cell-sub_title" :style="{ color: subTextColor, fontSize: subTextSize + 'rpx' }" v-if="subItem.subText">{{ subItem.subText }}</view>
          </view>
        </scroll-view>
      </swiper-item>
    </swiper>
